...
const LocateMeButton = ({
  ...
  return (
    <button
      onClick={locateUser}
      className="absolute right-3 top-3 z-[1000] flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm shadow"
    >
      <LuLocateFixed size={16} />
      Use current location
    </button>
...
const AddAddressPage = () => {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
      ...
      {/* LIST */}
      {addresses.map((a) => (
        <div key={a._id} className="border p-3 flex justify-between">
          <div>
            <p>{a.formattedAddress}</p>
            <p>{a.mobile}</p>
          </div>
