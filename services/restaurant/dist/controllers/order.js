...
//   });
//   } catch (error: any) {
//     console.log("Rider Assigned error ", error);
//     res.status(500).json({
//       message:
...
export const getCurrentOrderForRider = TryCatch(async (req, res) => {
    ...
    console.log("getCurrentOrderForRider ==controller", order);
...
export const updateOrderStatusRider = TryCatch(async (req, res) => {
    ...
    const order = await Order.findById(orderId);
    console.log("order controlles", order);
    if (!order) {
        return res.status(404).json({
            message: "Order not found",
        });
